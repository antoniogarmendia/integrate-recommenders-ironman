package ecore.er.recommender.sirius.actions;
...
import ecore.er.recommender.sirius.utils.RecommenderCase;
import ecore.er.recommender.sirius.dialog.RecommenderData;
import static ecore.er.recommender.sirius.utils.RecommenderUtils.*;
import java.util.Arrays;
import ecore.er.recommender.sirius.dialog.RecommenderDialog;
...
public class Recommendattributes implements IExternalJavaAction {
	public Recommendattributes() {
	}
	@Override
	...
	if (!selectedElements.isEmpty()) {
		...
		if (selectedNode instanceof DNodeList) {
			...
			if (targetEObject instanceof Entity) {
				final Entity target = ((Entity) targetEObject);
				final EClassifier eClassifier = target.eClass().getEStructuralFeature("attributes").getEType();
				...
				if (dataFusion.size() != 0 ) {
					...
					final RecommenderDialog recDialog = new RecommenderDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell()
							, recServerToItemRecommenders, normalizeDataFusion, "Entity", "EAttribute");
	...
	private RecommenderCase getRecommenderCase(String targetName, EClassifier eClassifier, Entity target) {
	...
	private void addSelectRecommendation(List<RecommenderData> selectedRecommendations, 
		...
		selectedRecommendations.stream().forEach(rec -> {
			final EClassifier eClassifierRead = target.eClass().getEStructuralFeature("attributes").getEType();
			...
			//Write
			EStructuralFeature structFeateAllAttributes = target.eClass().getEStructuralFeature("ownedAttributes");
	...
	private String makeBody(Entity target, String targetName) {
	...
	private Target getTarget(Entity target, String targetName) {
		final Target targetRequest = new Target();
		targetRequest.setName(targetName);
		final EStructuralFeature readStruct = target.eClass().getEStructuralFeature("attributes");

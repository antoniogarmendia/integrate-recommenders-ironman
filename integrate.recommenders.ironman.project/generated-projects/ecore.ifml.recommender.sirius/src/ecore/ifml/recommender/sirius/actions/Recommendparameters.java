package ecore.ifml.recommender.sirius.actions;
...
import ecore.ifml.recommender.sirius.utils.RecommenderCase;
import ecore.ifml.recommender.sirius.dialog.RecommenderData;
import static ecore.ifml.recommender.sirius.utils.RecommenderUtils.*;
import java.util.Arrays;
import ecore.ifml.recommender.sirius.dialog.RecommenderDialog;
...
public class Recommendparameters implements IExternalJavaAction {
	...
	if (!selectedElements.isEmpty()) {
		...
		if (selectedNode instanceof DSemanticDecorator) {
			...
			if (targetEObject instanceof ViewContainer) {
				final ViewContainer target = ((ViewContainer) targetEObject);
				final EClassifier eClassifier = target.eClass().getEStructuralFeature("parameters").getEType();
				...
				if (dataFusion.size() != 0 ) {
					...
					final RecommenderDialog recDialog = new RecommenderDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell()
							, recServerToItemRecommenders, normalizeDataFusion, "ViewContainer", "EAttribute");
	...
	private RecommenderCase getRecommenderCase(String targetName, EClassifier eClassifier, ViewContainer target) {
	...
	private void addSelectRecommendation(List<RecommenderData> selectedRecommendations, 
		...
		selectedRecommendations.stream().forEach(rec -> {
			final EClassifier eClassifierRead = target.eClass().getEStructuralFeature("parameters").getEType();
			...
			//Write
			EStructuralFeature structFeateAllAttributes = target.eClass().getEStructuralFeature("parameters");
	...
	private String makeBody(ViewContainer target, String targetName) {
	...
	private Target getTarget(ViewContainer target, String targetName) {
		final Target targetRequest = new Target();
		targetRequest.setName(targetName);
		final EStructuralFeature readStruct = target.eClass().getEStructuralFeature("parameters");
